...
// Default values - EXTREME VALUES FOR MAXIMUM RANGE
#define SERVO_PIN RC2           // Servo on RC2/CCP1
#define SERVO_MIN_POS_DEFAULT 40  // Absolute minimum position (~0.4ms pulse)
#define SERVO_MID_POS_DEFAULT 150 // Middle position (~1.5ms pulse)
#define SERVO_MAX_POS_DEFAULT 255 // Absolute maximum position (~2.55ms pulse)

// Movement parameters
#define SERVO_STEP 5            // Step size for smooth movement
#define SERVO_DELAY 30          // Delay between steps in milliseconds
#define SERVO_SETTLE_TIME 1000  // Longer settle time to prevent swerving
...
void initialize(void) {
    ...
    PR2 = 249;      // PWM Period = 4ms (250Hz)
                    // Formula: PWM Period = (PR2+1) * 4 * Tosc * Prescaler
                    // 4ms = (249+1) * 4 * 0.25μs * 16
...
void setServoPosition(uint8_t position) {
    // No range checking - allow extreme values
...
void moveToPosition(uint8_t position) {
    ...
    if (position > currentPosition + 2 || position < currentPosition - 2) {
        ...
        // Allow servo to fully stabilize with longer delay
